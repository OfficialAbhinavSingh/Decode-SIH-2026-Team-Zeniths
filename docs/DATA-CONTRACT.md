...
## National coverage extension (additive — the MVP contract above is unchanged)

Added for pan-India coverage. Every field below is **nullable** or a **new table** — no
column above changed type, dropped, or became required, and every endpoint above keeps
its exact request/response shape for the default city. `zone_id` stays the only join key
zones/signals/scores need; the tables below add optional context, not a new keying scheme.

### New tables

**`cities`** — owned by R2. One row per urban local body (`pipelines/geo/registry.py`,
built from the GeoNames India dump). `code` (e.g. `JAI`) is the prefix of every zone_id
generated for that city. `zones.city_code` joins here.

**`groundwater_stress`** — owned by R2. One row per state (district nullable, unused
today), from CGWB's *Dynamic Ground Water Resources of India, 2023*. Keyed by
`(state, district)`, joined to a zone through `zones.state`. Feeds fusion as an **urgency
lift on the leak score** (`services/urgency.py`) — never as a fourth signal, since
groundwater stress is not evidence a zone is leaking.

**`rainfall_observations`** — owned by R2. One row per city per day, from Open-Meteo/ERA5.
Joined through `zones.city_code`. Down-weights the satellite signal's fusion weight during
heavy recent rain (`services/rain.py`) so a monsoon week isn't read as a citywide leak.

**`city_scores`** — owned by R3. National rollup, one row per city per fusion run,
recomputed by the same pass that writes `zone_scores`. What `GET /api/national/cities`
serves.
...
| Table | Column | Notes |
|---|---|---|
| `zones` | `city_code`, `state`, `area_km2` | nullable; unset for zones loaded before this extension |
| `zone_scores` | `absolute_score` | the weighted, coverage-discounted score *before* percentile ranking — comparable across cities, unlike `fusion_score` |
| `zone_scores` | `priority_score`, `urgency_multiplier` | `absolute_score` lifted by groundwater urgency; see `services/urgency.py` for why this is a bounded additive lift, not a multiplier that can tie at 100 |
| `zone_scores` | `groundwater_stress_pct`, `groundwater_category` | copied onto the score row at compute time so the API never needs a second join to explain a rank |
| `zone_scores` | `rain_flagged`, `rain_mm_7d` | whether/how much recent rain discounted this zone's satellite weight |
| `zone_scores` | `water_at_risk_kld` | recoverable kilolitres/day, from `services/impact.py` |

### New endpoints

| Method | Path | Returns |
|---|---|---|
| `GET` | `/api/national/summary` | one aggregate object — cities/zones/states scored, water at risk, the impact-ledger figures |
| `GET` | `/api/national/states` | one row per state, ranked by its worst city — the country-zoom choropleth |
| `GET` | `/api/national/cities?state=&limit=&min_priority=` | one row per city — the country-zoom bubble layer |
| `GET` | `/api/national/cities/{city_code}` | one city's rollup plus its worst 10 zones |
| `POST` | `/api/fusion/run/national?limit_cities=` | recomputes every zone in the country and re-ranks cities against each other, in one pass |

`GET /api/scores` and `GET /api/scores/geojson` both additionally accept `city_code=` (in
place of `city=`) and return every new `zone_scores` field above, still without breaking
any existing consumer that only reads the original fields.
